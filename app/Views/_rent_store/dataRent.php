        ...
        <div class="flex h-full bg-white">
            ...
            <div id="Sidebar" class="hidden bg-white lg:block w-72 h-full overflow-auto no-scrollbar">
                <ul class="menu  menu-lg w-full rounded-box drop-shadow-2xl">
                    ...
                    <!-- Tambahkan menu untuk data busana, jenis busana, dan paket busana -->
                    <li><a href="<?= base_url('/rentbusana'); ?>" class="menu-item">Daftar Busana</a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <section class="h-screen p-5 w-full overflow-auto bg-slate-100">
        <div class="pt-20 mx-auto flex flex-wrap h-full max-w-full gap-10">
            ...
            <?php foreach ($list_busana as $item) : ?>
                <div style="width: 250px; height: 50%;" class="card card-compact w-80 bg-base-100 shadow-xl">
                    <?php $img = ($item['photo_busana'] == null) ? 'default_busana_2.jpeg' : $item['photo_busana'] ?>
                    <figure class="h-52 overflow-hidden"><img class="object-cover w-full h-full" src="<?= base_url('/uploads/') . $img; ?>" alt="Busana" /></figure>
                    <div class="card-body">
                        ...
                        <div class="card-actions justify-end">
                            <?php if ($item['stok'] != null) : ?>
                                <a href="<?= base_url('/setRental/' . $item['id_busana']); ?>" class="btn btn-neutral">Sewa Sekarang</a>
                            <?php else : ?>
